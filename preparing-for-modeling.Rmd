...
locs <- read_csv("all-ngsschat-locations.csv")

locs$state <- maps::map.where("state", x = locs$lon, y = locs$lat)

locs$state <- ifelse(str_detect(locs$location, "alaska"), "alaska",
                     ifelse(str_detect(locs$location, "hawaii"), "hawaii", locs$state))

locs <- distinct(locs, location, .keep_all = TRUE)
...
d$state <- d$state %>% 
  ...
  str_remove("virginia:chesapeake") %>% 
  str_remove("washington:san juan island")
```
...
```{r, desciptive-stats}
d %>% 
  select(scale_neg, scale_pos, senti_scale, senti_binary, state, screen_name, year, near_adoption, before_adoption, after_adoption, no_adoption) %>% 
  skimr::skim()
```
...
```{r}
write_rds(rtweet::flatten(d), 'ngsschat-data-for-modeling.rds')
```
