...
sjPlot::tab_model(m, show.icc = TRUE, show.ci = FALSE, show.se = TRUE, show.std = "std2")
...
```{r}
augmented_data <- broom.mixed::augment(m)
write_rds(augmented_data, "augmented_data.rds")
```

```{r, eval = FALSE}
augmented_data %>% 
  group_by(year_of_post_centered) %>% 
  summarize(mean_scale = mean(senti_scale)) %>% 
  ggplot(aes(x = year_of_post_centered, y = mean_scale)) +
  geom_point() +
  theme_bw()
```