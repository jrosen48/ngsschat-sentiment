...
```{r}
data_to_model %>% group_by(year_of_post_centered) %>% summarize(mean_senti = mean(senti_scale)) %>% ggplot(aes(y = mean_senti, x = year_of_post_centered)) + geom_point() + geom_smooth()
```
