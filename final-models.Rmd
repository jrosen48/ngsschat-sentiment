...
processing adoption status

```{r}
d <- d %>% 
  mutate(adoption_key = ifelse(is.na(adoption_key), "missing", adoption_key))
```
...
m_senti_scale <- lmer(senti_scale ~ 1 + 
                        ...
                        adoption_status + # status of an individual's state regarding when they adopted the NGSS
